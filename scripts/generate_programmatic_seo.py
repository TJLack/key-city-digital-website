...
def build_page(service, industry, city, mode="service-industry-city"):
    ...
    related_cities = city["nearby_cities"][:3]

    nearby_links = ''.join(
        (f'<li><a href=\"../../{slugify(n)}-tx/\">{service["service"]} for {industry["industry"]} in {n}, TX</a></li>'
         if mode == "service-industry-city"
         else f'<li><a href=\"../{slugify(n)}-tx/\">{service["service"]} in {n}, TX</a></li>')
        for n in related_cities
    )
