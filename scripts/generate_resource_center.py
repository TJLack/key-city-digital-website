...
def make_article(article: dict, all_articles: list[dict]) -> str:
    ...
    pseo_link = f'..{article["pseo_url"]}index.html'
...
def make_hub(articles: list[dict]) -> str:
    ...
    for a in articles:
        cards.append(
            f'<article class="card"><p class="kicker">{slug_to_label(a["cluster"])}</p><h3><a href="{a["slug"]}.html">{a["title"]}</a></h3><p>{slug_to_label(a["type"])} for {a["industry"].title()} teams in {a["primary_city"].replace("-", " ").title()}.</p><p class="small"><a href="../services/{a["service_slug"]}.html">Related service</a> · <a href="..{a["pseo_url"]}index.html">Related local page</a></p></article>'
        )
